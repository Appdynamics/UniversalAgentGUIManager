...
/**
 * Class CreateRulebook handles the GUI components required to create a rulebook
 * Is also responsible to forward user requests to the Rest Controller that will
 * execute the requests
 * 
 * @author nikolaos.papageorgiou
 *
 */
public class CreateRulebookNew {
	private JFrame frame;
	private RestController controller;
	private JPanel rulePanel = new JPanel();

	public CreateRulebookNew(RestController controller) {
	...
	private void initialize() {
		...
		JPanel rulebookPanel = new RulebookGUIFactory().rulebookPanel(Configuration,
				rulebookConfigurationAttributes.getAttributeNames(),
				rulebookConfigurationAttributes.getAttributesMap());
		...
		rulePanel.setLayout(new CardLayout(0, 0));
						...
						if (procceed) {
							...
							if (resultCode >= 200 && resultCode < 220) {
								JOptionPane.showMessageDialog(frame,
										rulebook.getName() + " has been successfully created!", "Successful Creation",
