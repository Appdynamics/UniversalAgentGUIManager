...
public class UpdateGroup {
	...
	public void run(final RestController controller, final String selectedGroup) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UpdateGroup window = new UpdateGroup(controller, selectedGroup);
	...
	public UpdateGroup(RestController controller, String selectedGroup) {
	...
	private void initialize() {
		...
		try {
			Group group = controller.getGroupByName(selectedGroup);
			groupName = group.getName();
			groupComments = group.getComments();
		...
		textGroupComments.setText(groupComments);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (resultCode == 200) {
					JOptionPane.showMessageDialog(frame, "Group " + group.getName() + " has been created!",
							"Group Updated Successfully", JOptionPane.INFORMATION_MESSAGE);
					frame.dispose();
				} else {
					String result = "The resulted Code was: " + resultCode;
					JOptionPane.showMessageDialog(null, "The group update failed", result, JOptionPane.ERROR_MESSAGE);
				}
