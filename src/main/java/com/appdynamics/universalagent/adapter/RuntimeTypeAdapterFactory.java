	...
	public <R> TypeAdapter<R> create(Gson gson, TypeToken<R> type) {
		...
		return new TypeAdapter<R>() {
			...
			public void write(JsonWriter out, R value) throws IOException { // Unimplemented as we don't use write.
				/*
				 * Class<?> srcType = value.getClass(); String label =
				 * subtypeToLabel.get(srcType);
				 * 
				 * @SuppressWarnings("unchecked") // registration requires that subtype extends
				 * T TypeAdapter<R> delegate = (TypeAdapter<R>) subtypeToDelegate.get(srcType);
				 * if (delegate == null) { throw new JsonParseException("cannot serialize " +
				 * srcType.getName() + "; did you forget to register a subtype?"); } JsonObject
				 * jsonObject = delegate.toJsonTree(value).getAsJsonObject(); if
				 * (jsonObject.has(typeFieldName)) { throw new
				 * JsonParseException("cannot serialize " + srcType.getName() +
				 * " because it already defines a field named " + typeFieldName); } JsonObject
				 * clone = new JsonObject(); clone.add(typeFieldName, new JsonPrimitive(label));
